...
function normalizeConjStats(raw: unknown): ConjStats {
  const p = (raw ?? {}) as Partial<ConjStats>;
  return {
    ...EMPTY_STATS,
    ...p,
    byForm: p.byForm ?? {},
...
/** Saved to the learner's account when signed in (see useSyncedJson). */
export function useConjStats() {
  const [stats, setStats, hydrated] = useSyncedJson("conj", EMPTY_STATS, normalizeConjStats);
