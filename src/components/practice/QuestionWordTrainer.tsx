...
export function QuestionWordTrainer() {
  const { t, lang } = useSettings();
  const stats = usePracticeStats("qwords");

  const [settings, setSettings] = useState<QWSettings>(DEFAULT_SETTINGS);
  ...
  const [queue, setQueue] = useState<QWDrill[]>([]);
  ...
  const options = useMemo(
    () => (item ? optionsFor(item, settings.words) : []),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [pos, item?.key],
  );

  const start = useCallback(
    (opts?: { weak?: boolean; only?: QWDrill[] }) => {
      const q =
  ...
  function choose(w: string) {
    ...
    if (ok) {
      ...
      setStreak(0);
      setWrong((w2) => (w2.some((x) => x.key === item.key) ? w2 : [...w2, item]));
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="space-y-4">
        ...
        >
          {options.map((w) => {
            const isAnswer = !!result && w === item.answer;
